...
for f in os.listdir('./'):
  metric = ['f1', 'auc', 'prec', 'reca', 'acc', 'loss']
  for m in metric:
    if f.endswith('.hdf5') and f.startswith(savename+m):
      ...
      #np.savetxt('labelassignyfold0.txt', y_pred_val)
      y_pred_val = y_pred_val.max(axis=1) #np.sort(y_pred_val, axis=1) #y_pred_val.max(axis=1)
      #y_pred_val = np.mean(y_pred_val[:,:mymilk], axis=1)
      y_test = Y_test_test.max(axis=1)
      ...
      y_pred = y_pred.max(axis=1)#np.sort(y_pred, axis=1) #y_pred_val.max(axis=1)
      #y_pred = np.mean(y_pred[:,:mymilk], axis=1)
      score = roc_auc_score(Y_test_test.max(axis=1), y_pred) #y_pred.max(axis=1)) 
      fpr, tpr, _ = roc_curve(Y_test_test.max(axis=1), y_pred)
      #np.savetxt('fpr.txt', fpr)
      #np.savetxt('tpr.txt', tpr)
      y_true = Y_test_test.max(axis=1)
      y_score = y_pred > bestthre #y_pred.max(axis=1)>bestthre
